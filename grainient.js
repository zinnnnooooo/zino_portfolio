import { Renderer, Program, Mesh, Triangle } from './ogl.esm.js';
...
export function initGrainient(targetContainer, options = {}) {
  if (!targetContainer) return null;

  let container = targetContainer;
  if (!container.classList.contains('grainient-container')) {
    let wrapper = container.querySelector('.grainient-container');
    if (!wrapper) {
      wrapper = document.createElement('div');
      wrapper.className = 'grainient-container';
  ...
  const defaults = {
    timeSpeed: isReducedMotion ? 0.0 : 0.16,
    colorBalance: 0.0,
    warpStrength: 0.9,
    warpFrequency: 4.0,
    warpSpeed: 1.1,
    ...
    centerY: 0.0,
    zoom: 0.9,
    color1: '#FF8AF4', // Pink / Magenta
    color2: '#5227FF', // Electric Purple
    color3: '#2563FF', // Electric Blue
    lightMode: false,
  };
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uLightMode:      { value: settings.lightMode ? 1.0 : 0.0 }
    }
  });
  ...
  window.__grainientDebug = {
    getITime: () => program.uniforms.iTime.value,
    getResolution: () => program.uniforms.iResolution.value,
    isRafRunning: () => raf !== 0,
    program,
    renderer
  };
